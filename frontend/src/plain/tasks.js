...
const RULES = {
  ...
  // 여기서 가장 급한 것. 본인 여부를 물을 일이 아니다 — 정상적인 프로그램은 이렇게 동작하지 않는다.
  // 그래서 JUDGE 가 아니라 순서를 주는 GUIDE 다. 대신 되돌릴 수 없는 일은 시키지 않는다.
  // 프로그램을 죽이라거나 파일을 지우라고 하지 않고, 컴퓨터를 끄지 말라고 먼저 말한다 —
  // 비전문가가 급히 정리하면 침입의 단서까지 함께 사라진다.
  proc_shell_over_socket: {
    kind: KIND.GUIDE, icon: 'alert',
    title: () => '외부에서 이 컴퓨터를 조종하려는 프로그램이 있어요',
    what: '이 컴퓨터에 명령을 내리는 창구가, 바깥으로 나가는 연결에 그대로 이어져 있어요. '
        + '바깥에 있는 누군가가 이 컴퓨터에 직접 명령을 내릴 수 있는 상태예요.',
    why: '평범한 프로그램은 이렇게 동작하지 않아요. 지금 남이 이 컴퓨터를 쓰고 있을 수 있어요.',
    steps: [
      '인터넷 선을 뽑으세요. 무선이면 와이파이를 끄세요. 바깥에서 오는 연결을 끊는 것이 가장 먼저 할 일이에요.',
      '컴퓨터를 끄거나 다시 시작하지는 마세요. 끄면 무슨 일이 있었는지 알아낼 단서가 사라져요.',
      '휴대폰처럼 다른 기기로, 이 컴퓨터에서 쓰던 계정의 비밀번호를 바꾸세요.',
      '이 화면을 사진으로 찍어 두세요. 도움을 받을 때 그대로 보여주시면 돼요.',
      '도움 받을 수 있는 사람에게 바로 알리세요. 혼자 판단하지 않으셔도 되는 일이에요.',
    ],
  },
  ...
  // ---- 컴퓨터 상태 (위생) ----
  //
  // 이 넷은 보통 침입과 상관이 없다. 그래서 겁주지 않고, 먼저 '흔한 이유'를 말한다.
  // 다만 저장 공간은 다르게 다룬다 — 공간이 다 차면 기록이 멈추고, 기록이 멈추면 지킴이가
  // 못 보게 된다. 그래서 '느려져요'가 아니라 '감시가 멈춰요'라고 적는다.
  disk_full: {
    kind: KIND.GUIDE, icon: 'box',
    title: () => '저장 공간이 거의 다 찼어요',
    what: '이 컴퓨터에 파일을 넣어둘 공간이 거의 남지 않았어요.',
    why: '공간이 다 차면 컴퓨터가 기록을 더 쓸 수 없어요. 기록이 멈추면 지킴이도 무슨 일이 있었는지 못 보게 돼요.',
    steps: [
      '내려받은 파일이나 안 보는 영상처럼 큰 파일부터 지워 공간을 만드세요.',
      '휴지통도 비우세요. 비우기 전까지는 공간이 돌아오지 않아요.',
      '무엇이 공간을 차지하는지 모르겠으면 이 화면을 도움 주실 분께 보여주세요.',
    ],
  },
  high_cpu: {
    kind: KIND.GUIDE, icon: 'chart',
    title: () => '컴퓨터가 계속 온 힘을 쓰고 있어요',
    what: '어떤 프로그램이 이 컴퓨터의 계산 능력을 한동안 거의 전부 쓰고 있어요. 무엇이 쓰고 있는지는 아래 ‘지킴이가 본 것’에 이름이 남아 있어요.',
    why: '영상을 만들거나 큰 파일을 다루는 중이면 자연스러운 일이에요. 그런 일을 하지 않았다면, 남이 이 컴퓨터에 다른 일을 시키고 있을 수도 있어요.',
    steps: [
      '지금 무거운 프로그램을 돌리고 계신가요? 그렇다면 그대로 두셔도 돼요.',
      '그런 적이 없다면 아래에서 프로그램 이름을 확인하세요.',
      '모르는 이름이면 이 화면을 도움 주실 분께 보여주세요.',
    ],
  },
  high_memory: {
    kind: KIND.GUIDE, icon: 'chart',
    title: () => '프로그램들이 쓰는 공간이 거의 다 찼어요',
    what: '프로그램이 일하는 동안 잠깐 쓰는 공간이 거의 남지 않았어요. 파일을 넣어두는 저장 공간과는 다른 공간이에요.',
    why: '이 공간이 부족하면 프로그램이 갑자기 닫히거나 컴퓨터가 느려져요. 쓰지 않는 것을 닫으면 대개 풀려요.',
    steps: [
      '쓰지 않는 프로그램과 인터넷 창을 닫아보세요.',
      '그래도 그러면 컴퓨터를 한 번 다시 시작하세요.',
      '아무것도 켜두지 않았는데도 그렇다면 이 화면을 도움 주실 분께 보여주세요.',
    ],
  },
  process_spike: {
    kind: KIND.GUIDE, icon: 'chart',
    title: () => '돌아가는 프로그램 수가 갑자기 늘었어요',
    what: '짧은 사이에 이 컴퓨터에서 돌아가는 프로그램 수가 크게 늘었어요.',
    why: '큰 프로그램을 설치하거나 처음 켤 때도 이렇게 돼요. 그런 적이 없다면 어떤 프로그램이 스스로를 계속 늘리고 있을 수 있어요.',
    steps: [
      '방금 프로그램을 설치하거나 무거운 작업을 시작하셨나요? 그렇다면 잠시 뒤 저절로 가라앉아요.',
      '그런 적이 없다면 컴퓨터를 한 번 다시 시작하세요.',
      '다시 시작한 뒤에도 같으면 이 화면을 도움 주실 분께 보여주세요.',
    ],
  },
  time_unsynced: {
    kind: KIND.GUIDE, icon: 'clock',
    title: () => '컴퓨터 시계가 실제 시각에 맞춰지지 않고 있어요',
    what: '컴퓨터는 보통 인터넷에서 정확한 시각을 받아 시계를 맞춰요. 지금은 그게 되지 않고 있어요.',
    why: '시각이 어긋나면 기록에 적히는 시간도 어긋나요. 나중에 무슨 일이 언제 있었는지 맞춰보기 어려워져요.',
    steps: [
      '설정에서 ‘날짜와 시간’을 열고, 시간을 자동으로 맞추는 설정이 켜져 있는지 보세요.',
      '켜져 있는데도 맞지 않으면 컴퓨터를 한 번 다시 시작해 보세요.',
      '그래도 그대로면 이 화면을 도움 주실 분께 보여주세요.',
    ],
  },
  ...
  // 지킴이 역할을 하던 프로그램이 사라진 것. 본인이 지웠는지는 본인만 안다.
  // 그래서 JUDGE 다 — 여기서 '복구하세요'라고 먼저 말하면, 본인이 일부러 지운 경우에
  // 틀린 지시가 된다. 아니라고 답하면 공통 안내(JUDGE_STEPS)로 이어진다.
  security_package_removed: {
    kind: KIND.JUDGE, icon: 'shield',
    title: (n) => (n > 1 ? `이 컴퓨터를 지키던 프로그램이 ${n}개 지워졌어요` : '이 컴퓨터를 지키던 프로그램이 지워졌어요'),
    what: '문을 두드리는 상대를 막아주거나, 무슨 일이 있었는지 기록해 주던 프로그램이 사라졌어요. '
        + '그만큼 지킴이가 볼 수 있는 범위도 줄었어요.',
    why: '직접 지우신 것이면 괜찮아요. 그런 적이 없다면, 감시를 없애려고 누가 먼저 지운 것일 수 있어요.',
    ask: '이 프로그램을 직접 지우셨나요?',
  },

  // 방화벽 기록을 보는 감시자가 올리는 셋. port_scan(두드려만 봄)과 다른 지점은
  // '그다음에 실제로 무언가를 했다'는 것이다. 그 차이를 문장에서 그대로 말한다.
  scan_then_auth: {
    kind: KIND.JUDGE, icon: 'alert',
    title: () => '문을 두드려 보던 상대가 로그인까지 시도했어요',
    what: '앞서 이 컴퓨터의 문을 여러 개 두드려 본 상대가, 그다음에 로그인을 시도했어요. '
        + '성공했는지 실패했는지는 아래 ‘지킴이가 본 것’에 그대로 남아 있어요.',
    why: '문을 두드려 보기만 한 것과는 다른 일이에요. 열린 문을 찾은 다음 실제로 들어오려 한 흐름이에요.',
    ask: '이 무렵 직접 접속하셨나요? (점검하는 프로그램을 돌려보신 적이 있나요?)',
  },
  scan_then_connection: {
    kind: KIND.JUDGE, icon: 'door',
    title: () => '문을 두드려 보던 상대가 실제로 연결했어요',
    what: '앞서 이 컴퓨터의 문을 여러 개 두드려 본 상대가, 그중 열려 있던 문으로 실제로 연결을 맺었어요. '
        + '어느 문이었는지는 아래 기록에 남아 있어요.',
    why: '내가 일부러 열어둔 문이고 내가 접속한 것이면 평소와 같은 일이에요. 그렇지 않다면 낯선 상대가 들어와 있다는 뜻이에요.',
    ask: '이 무렵 다른 기기에서 직접 접속하셨나요?',
  },
  internal_scan: {
    kind: KIND.JUDGE, icon: 'door',
    title: () => '같은 인터넷을 쓰는 기기가 이 컴퓨터의 문을 하나씩 두드려 봤어요',
    what: '집이나 사무실의 같은 인터넷에 연결된 다른 기기가, 이 컴퓨터의 문을 여러 개 확인해 봤어요. '
        + '어떤 기기였는지는 아래 기록에 주소가 남아 있어요.',
    why: '프린터나 외장 저장장치가 주변 기기를 찾는 중일 수도 있어요. 그렇지 않다면 그 기기가 나쁜 프로그램에 걸려 주변을 훑고 있을 수 있어요.',
    ask: '기기를 새로 연결하거나, 네트워크를 점검하는 프로그램을 쓰신 적이 있나요?',
  },
};
