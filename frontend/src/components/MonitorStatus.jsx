...
const MONITOR_INFO = {
    ...
    FirewallLogWatcher: {
        desc: 'ufw 가 막은 패킷 기록(/var/log/ufw.log)으로 포트 스캔을 봅니다. 스캔 자체는 이벤트와 하루 요약으로만 남깁니다.',
        details: ['같은 IP 가 60초 안에 막힌 포트 10개 이상(TCP SYN·UDP) → 포트 스캔 이벤트 (알림 아님)', '스캔한 IP 가 24시간 안에 SSH 로그인 시도·성공, 또는 리스닝 포트에 실제 연결 → 경고 알림', '내부망(사설 대역) 주소의 스캔 → 곧바로 경고 (감염된 기기일 수 있음)', 'ufw 로그는 속도 제한(분당 3건)이 있어 숫자는 최소치', '관리자 IP(SECDASH_F2B_IGNOREIP)와 루프백은 세지 않고, 자동 차단하지 않음', '로그를 읽을 수 없거나 ufw 가 꺼져 있으면 제한(DEGRADED)으로 표시'],
    },
    ProcessAudit: {
