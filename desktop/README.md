...
2. **설치** — 처음 한 번 내 계정 비밀번호를 묻는다(입력하는 글자는 보이지 않는다).
   ...
   ```
   설치하지 않고 쓰려면 같은 릴리스의 AppImage 를 받아 `chmod +x sentinel-overwatch_0.1.0_amd64.AppImage && ./sentinel-overwatch_0.1.0_amd64.AppImage`.
...
2. **설치** — `sudo apt install "./src-tauri/target/release/bundle/deb/Sentinel Overwatch_0.1.0_amd64.deb"`
